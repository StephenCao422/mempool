...
struct page_map {
  ...
  inline Span* get(page_id id) const noexcept {
  ...
  // writers must hold page_cache::_pageMtx
  inline void set(page_id id, Span* s) {
    const std::size_t i1 = id >> LEAF_BITS;
    const std::size_t i2 = id &  LEAF_MASK;
    Entry* leaf = root_[i1];
    if (!leaf) {
  ...
  inline void set_range(page_id start, std::size_t n, Span* s) {
